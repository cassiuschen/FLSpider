...
module.exports = function (day, next, force){
  ...
  var data = { gameCount:0, teamCount:0, bwinCount:0, match:{}, shortcut:{}, iid:{}, hid:{}, aid:{}, home:{}, away:{}, game:{}, team:{} };
  ...
  needle.defaults({ open_timeout:5000, read_timeout:10000 });
  ...
  var oddsStep = function (response) {
    ...
    tr.each(function (i,elm) {
      ...
      var j,current;
      if(ouzhiList[mid]){
        for(j=0;j<OUZHI_DICT.length;j++){
          current = OUZHI_DICT[j];
      ...
      if(yapanList[mid]){
        for(j=0;j<YAPAN_DICT.length;j++){
          current = YAPAN_DICT[j];
  ...
  var scoreStep = function (response) {
    ...
    tr.each(function (i,elm) {
      ...
      var hid = parser.int($(this).find('td').eq(3).find('a').attr('href').replace('http://liansai.500.com/team/','').replace('/'));
      var aid = parser.int($(this).find('td').eq(5).find('a').attr('href').replace('http://liansai.500.com/team/','').replace('/'));
      var obj = data.shortcut[shortcut]||data.hid[hid]||data.aid[aid];
      if(obj){
  ...
  var jingcaiLoop = function (){
    ...
    for(var j = 0; j<ql; j++){
      ...
      needle.get(URL.jingcai.replace('{iid}',q.obj.iid).replace('{type}',q.type).replace('{day}',day), {json:true,headers:{'X-Requested-With': 'XMLHttpRequest'}}, done(jingcaiStep(q)));
    }
  }
  var jingcaiStep = function (query){
    return function (response) {
      ...
      var obj = query.obj;
      var last = null;
      obj.jingcai[JINGCAI_DICT[query.type]] = [];
      for(var i = 0; i < odds.length; i++){
        //去掉重复赔率变化（早期数据可能出现此问题）
        if(!(last&&(odds[i].time === last.time && odds[i].win === last.win && odds[i].draw === last.draw && odds[i].lost === last.lost))){
          obj.jingcai[JINGCAI_DICT[query.type]].push({sp:[parser.number(odds[i].win),parser.number(odds[i].draw),parser.number(odds[i].lost)], time: new Date(odds[i].time) });
        }
        last = odds[i];
      }
      ep.emit('jingcai');
  ...
  var tradeStep = function (response) {
    if(response.body.xml&&response.body.xml.project.row){
      var parse = function (elm) {
        var no = elm.$;
        var obj = data.iid[parser.int(no.id)];
        if(obj&&obj.jingcai){
  ...
  var bwinStep = function (response) {
    ...
    tbody.each(function (i,elm) {
      ...
      var home = parser.trim($(this).find('tr').eq(0).find('td').eq(2).text());
      var away = parser.trim($(this).find('tr').eq(2).find('td').eq(0).text());
      var obj = data.shortcut[shortcut]||data.home[home]||data.away[away];
      if(!obj){
        data.bwinCount--;
      }
    });
    tbody.each(function (i,elm) {
      ...
      var home = parser.trim($(this).find('tr').eq(0).find('td').eq(2).text());
      var away = parser.trim($(this).find('tr').eq(2).find('td').eq(0).text());
      var bwin_home = parser.trim($(this).find('tr').eq(0).find('td').eq(10).text());
      var bwin_draw = parser.trim($(this).find('tr').eq(1).find('td').eq(8).text());
      var bwin_away = parser.trim($(this).find('tr').eq(2).find('td').eq(8).text());
      var obj = data.shortcut[shortcut]||data.home[home]||data.away[away];
      if(obj){
  ...
  needle.get(URL.odds.replace('{day}',day), {headers:{'Accept-Encoding': 'gzip, deflate, sdch'}}, done(oddsStep));
}
