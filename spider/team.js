...
module.exports = function (team, next){
  ...
  var hoa = 1;
  var data = { count:0, gameCount:0, teamCount:0, match:{}, game:{}, team:{} };
  ...
  var matchesStep = function (response) {
    ...
    var matches = JSON.parse(response.body).list;
    var fids = [];
    ...
    //抓取赔率完成
    ep.after('odds', CID_DICT.length, function () {
      //是否主客场都已抓取完毕
      if(hoa ==2){
        //保存事件响应
    ...
    //抓取赔率
    for(var j = 0; j < CID_DICT.length; j++){
      needle.get(URL.odds.replace('{cid}',CID_DICT[j].id).replace('{fids}',fids.join(',')), done(oddsStep(CID_DICT[j])));
    }
  }
  //抓取指定公司的赔率数据
